...
- Sources are listed in `sources.txt`, one per line, with how to read each one. Venues rarely publish feeds, so the build uses the most dependable thing each offers: AEG's event data (Roadrunner), an event feed (The Sinclair), schema.org event data (the Brattle), iCalendar (Cambridge Arts), or the page itself (the Coolidge, TicketWeb listings at the Middle East). A source that fails is named at the bottom of the page; the rest still show.
